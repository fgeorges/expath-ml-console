   ...
   as element()+
{
   ...
   let $value    := t:mandatory-field('value')
   ...
   return
      if ( fn:not(a:exists-on-database($db, $uri)) ) then (
         ...
         if ( $redirect ) then (
            ...
            <p>Metadata "<code>{ $key }</code>" successfully added to
               "<code>{ $uri }</code>".</p>,
